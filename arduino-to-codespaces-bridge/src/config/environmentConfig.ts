...
/**
 * A learned device mapping: a USB VID:PID pair proven (by a successful
 * upload) to be a specific board. Key format: "0xVVVV:0xPPPP".
 */
...
/** Line format of a learned device entry: device 0xVVVV:0xPPPP <fqbn> */
const DEVICE_KEY_RE = /^0x[0-9a-fA-F]{4}:0x[0-9a-fA-F]{4}$/;
...
function parseRequirementsTxt(content: string): EnvironmentConfig {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("platform ")) {
      ...
      if (parts.length >= 2 && DEVICE_KEY_RE.test(parts[0])) {
        devices.push({ key: parts[0].toLowerCase(), fqbn: parts[1] });
      }
    }
  }
  ...
  return {
    ...
    devices: sortDevices(dedupeDevices(devices)),
  };
}
...
/**
 * Sort learned device entries by key for merge-friendly output.
 *
 * @param devices The device entries to sort.
 * @returns A new sorted array.
 */
function sortDevices(devices: DeviceRequirement[]): DeviceRequirement[] {
  return [...devices].sort((a, b) => a.key.localeCompare(b.key));
}

/**
 * Collapse duplicate device keys - the LAST entry in file order wins
 * (matches the latest-successful-upload-wins policy).
 *
 * @param devices Parsed device entries in file order.
 * @returns Deduplicated entries.
 */
function dedupeDevices(devices: DeviceRequirement[]): DeviceRequirement[] {
  const map = new Map<string, DeviceRequirement>();
